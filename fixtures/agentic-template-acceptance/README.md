...
2. **`/plan`** (Phase C critique). Optional `/cold-review` only if listed in `grok inspect`.
...
2. Run `python -m pytest tests/test_util.py -q` then **`/implement`** accuracy (targeted + review)
...
2. **`/implement`** accuracy → targeted loop with coverage
...
2. Run **`/implement`** accuracy → targeted unit loop with `gf-qa`
...
2. **`/implement`** accuracy protocol — UI changed, so the UI verification step is required; judge must read `.grok/docs/ui-design-standards.md` and emit a **UI Verification Report** (see implement skill)
